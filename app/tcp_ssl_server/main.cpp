...
auto in_connections = [](stream_ptr &&stream, tcp::ssl::listen::settings::in_conn_handler_data_cb data) {
  if (!stream->is_active()) {
    std::cerr << "fail to create incomming connection " << stream->get_detailed_error() << std::endl;
    return;
  }
  auto *linux_stream = dynamic_cast<tcp::ssl::send::settings const *>(stream->get_settings());
...
int main(int argc, char **argv) {
  ...
  tcp::ssl::send::statistic client_stat;
  ...
  while (std::chrono::system_clock::now() < endTime && listen_stream->is_active()) {
    ...
    if (!cdata._need_to_handle.empty()) {
      ...
      if (!(*it)->is_active()) {
        client_stat += *static_cast<tcp::ssl::send::statistic const *>((*it)->get_statistic());
  ...
  for (auto &strm : cdata._streams) {
    client_stat += *static_cast<tcp::ssl::send::statistic const *>(strm.first->get_statistic());
  }

  auto const *stream_stat = static_cast<tcp::ssl::listen::statistic const *>(listen_stream->get_statistic());
