...
void thread_fun(proto::ip::address const &server_addr,
                ...
                proto::ip::address const &bind_addr,
                uint16_t bind_port,
                std::atomic_bool &work,
                size_t connections_per_thread,
                size_t thread_number,
                udp::send::statistic &stat,
                size_t data_size) {
  udp::send::settings settings;

  ev::factory manager;
  settings._peer_addr = {server_addr, server_port};
  settings._self_addr = {bind_addr, bind_port};
  ...
  while (work.load(std::memory_order_acquire)) {
    ...
    if (!need_to_handle.empty()) {
      ...
      if (!(*it)->is_active()) {
        stat += *static_cast<udp::send::statistic const *>((*it)->get_statistic());
  ...
  for (auto &strm : stream_pool) {
    stat += *static_cast<udp::send::statistic const *>(strm.first->get_statistic());
  }
}

int main(int argc, char **argv) {
  CLI::App app{"udp_client"};
  std::string server_address_string;
  uint16_t server_port;
  std::string bind_address_string;
  uint16_t bind_port;
  ...
  app.add_option("-q,--baddress", bind_address_string, "bind address")->required();
  app.add_option("-w,--bport", bind_port, "bind port")->required();
  ...
  proto::ip::address bind_address(bind_address_string);
  if (server_address.get_version() == proto::ip::address::version::e_none
      || bind_address.get_version() == proto::ip::address::version::e_none) {
