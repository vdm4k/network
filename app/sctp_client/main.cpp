...
void thread_fun(proto::ip::address const &server_addr, uint16_t server_port,
                ...
                sctp::send::statistic &stat, ev_stream_factory &manager,
  ...
  while (work.load(std::memory_order_acquire)) {
    ...
    if (!_need_to_handle.empty()) {
      ...
      if (!(*it)->is_active()) {
        stat +=
            *static_cast<sctp::send::statistic const *>((*it)->get_statistic());
...
int main(int argc, char **argv) {
  ...
  for (auto &wrk : worker_pool) {
    ...
    for (auto &strm : wrk._stream_pool) {
      stat += *static_cast<sctp::send::statistic const *>(
