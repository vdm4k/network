...
void thread_fun(proto::ip::address const &server_addr,
  ...
  while (work.load(std::memory_order_acquire)) {
    ...
    if (!need_to_handle.empty()) {
      ...
      if (!(*it)->is_active()) {
        stat += *static_cast<udp::ssl::send::statistic const *>((*it)->get_statistic());
  ...
  for (auto &strm : stream_pool) {
    stat += *static_cast<udp::ssl::send::statistic const *>(strm.first->get_statistic());
  }
}

int main(int argc, char **argv) {
  CLI::App app{"udp_ssl_client"};
