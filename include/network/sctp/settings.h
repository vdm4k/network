 ...
 */
  ...
  uint16_t _srto_min
    = 1000; ///< Value under which the RTO does not descend, we set this value to not conflict with srto_max
  ...
  uint16_t _sinit_max_init_timeo
    = 10000; ///< the largest Time-Out or RTO value (in milliseconds) to use in attempting an INIT.
  ...
  uint8_t _sctp_partial_delivery_event
    = 1; ///< a partial delivery is aborted, probably indicating the connection is being shutdown
