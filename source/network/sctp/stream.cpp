...
/* Set the association parameters: max number of retransmits, ... */
  ...
  assoc.sasoc_asocmaxrxt =
      params->_sasoc_asocmaxrxt; /* Maximum number of retransmission attempts:
...
/* Set the INIT parameters, such as number of streams */
  ...
  init.sinit_num_ostreams =
      params->_sinit_num_ostreams; /* desired number of outgoing streams */
...
/* The SO_LINGER option will be reset if we want to perform SCTP ABORT */
  ...
  event.sctp_data_io_event =
      params->_sctp_data_io_event; /* to receive the stream ID in SCTP_SNDRCV
                                      ancilliary data on message reception */
  event.sctp_association_event =
      params->_sctp_association_event; /* new or closed associations (mostly for
  ...
  event.sctp_send_failure_event =
      params->_sctp_send_failure_event; /* delivery failures */
  event.sctp_peer_error_event =
      params->_sctp_peer_error_event; /* remote peer sends an error */
  event.sctp_shutdown_event =
      params->_sctp_shutdown_event; /* peer has sent a SHUTDOWN */
  event.sctp_partial_delivery_event =
      params->_sctp_partial_delivery_event; /* a partial delivery is aborted,
 ...
 * a default for all addresses in the association; not sure if it works ... */
  ...
  if (params->_enable_heart_beats) {
    ...
    parms.spp_flags |=
        SPP_PMTUD_ENABLE; /* also enable path MTU discovery mechanism */
#endif                    /* SPP_PMTUD_ENABLE */
    parms.spp_hbinterval =
        params->_spp_hbinterval; /* Send an heartbeat every 6 seconds to quickly
